...
void USB_printf(const char *format, ...)
{
	...
	int32_t length;				// 数据的长度
	uint16_t	 USB_TimeOut;		// 超时判断时间
	uint32_t  Tickstart;			// 计时起始时间

	if ((format == NULL) || (hUsbDeviceFS.dev_state != USBD_STATE_CONFIGURED) || (hUsbDeviceFS.pClassData == NULL))
	{
		return;
	}
	...
	if ((uint32_t)length >= APP_TX_DATA_SIZE)
	{
		length = APP_TX_DATA_SIZE - 1U;
	}
